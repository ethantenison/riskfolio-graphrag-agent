...
### Q: Why is graph mode returning few results?

**A**: Graph mode depends on well-populated `Assertion` nodes and `CanonicalEntity` nodes. If the Neo4j graph lacks assertion coverage, bridge expansion and hop expansion will have little to return. Check:
- Do `Assertion` nodes exist? `MATCH (a:Assertion) RETURN count(a)`
- Are they linked to chunks? `MATCH (a:Assertion)-[:SUPPORTED_BY]->(c:Chunk) RETURN count(a)`
- Are `CanonicalEntity` nodes present? `MATCH (e:CanonicalEntity) RETURN count(e)`

### Q: Why is sparse mode returning irrelevant results?

**A**: Sparse relies purely on keyword overlap. If the query includes common words that appear in unrelated contexts, unrelated chunks may rank high. Use graph or hybrid mode for semantic filtering.

### Q: Dense results are good but slow. Can I speed it up?

**A**: Yes. Switch to dense mode with a smaller `top_k` or use a faster embedding model (e.g., lexical embeddings instead of semantic). Dense mode does not require the Neo4j graph, only the vector backend.

### Q: Why is the final score different than the merged score?

**A**: Reranking boosts are applied after merging. The final score is typically `0.70–0.88 × merged_score + evidence_boost`. This is intentional; graph context should influence ranking.
